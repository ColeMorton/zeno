...
/// @title VarianceVaultLong
/// @notice ERC-4626 vault providing long volatility exposure via variance swaps
/// @dev Users deposit collateral, receive shares, auto-matched with short vault
contract VarianceVaultLong is ERC4626, IVarianceVault {
    ...
    /// @param _counterpartyVault VarianceVaultShort address
    ...
    constructor(
        ...
        ERC4626(_asset)
        ERC20("Variance Vault Long vBTC", "vvLongVBTC")
    {
    ...
    function matchEpoch(uint256 epochId) external {
        ...
        uint256 collateralRequired = IVarianceSwap(varianceSwap).calculateLongCollateral(terms);

        uint256 swapId = IVarianceSwap(varianceSwap).createLongSwap(terms, collateralRequired);
    ...
    function settleEpoch(uint256 epochId) external {
        ...
        emit EpochSettled(epochId, result.realizedVariance, result.pnl);
    }
    ...
    function _getEpochCurrentValue(uint256 epochId) internal view returns (uint256) {
        ...
        if (epoch.state == EpochState.DEPOSITING) {
            ...
            // Could estimate based on current variance, but simpler to return deposits
            return epoch.totalDeposits;
