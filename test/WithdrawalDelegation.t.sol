...
contract WithdrawalDelegationTest is Test {
    ...
    function test_WithdrawAsDelegate_ZeroAmount() public {
        ...
        // Drain the vault collateral to near zero
        vm.startPrank(alice);
        ...
        // Bob's withdrawal should return 0 or very small amount
        vm.prank(bob);
        uint256 withdrawn = vault.withdrawAsDelegate(tokenId);
        assertLt(withdrawn, ONE_BTC / 1000); // Less than 0.001 BTC
    }
    ...
    function testFuzz_DelegationPercentages(uint256 percentage1, uint256 percentage2) public {
        ...
        vm.prank(bob);
        uint256 bobWithdrawn = vault.withdrawAsDelegate(tokenId);
        uint256 expectedBob = (totalPool * percentage1) / 10000;
        assertEq(bobWithdrawn, expectedBob);

        vm.prank(charlie);
        uint256 charlieWithdrawn = vault.withdrawAsDelegate(tokenId);
        uint256 expectedCharlie = (totalPool * percentage2) / 10000;
        assertEq(charlieWithdrawn, expectedCharlie);

        // Total should match
        assertEq(bobWithdrawn + charlieWithdrawn, expectedBob + expectedCharlie);
    }
