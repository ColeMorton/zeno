Run the simulation skill to execute a **512-week** (full) swarm simulation.

## Steps

### 0. Price data reuse

Check if `.claude/skills/simulation/data/price_series.csv` exists. If so, copy it to the Forge sandbox before running:

```bash
cp .claude/skills/simulation/data/price_series.csv contracts/simulation/reports/price_series.csv
```

If it does not exist, the simulation will generate prices live via GBM.

### 1. Run simulation

```bash
cd "$(git rev-parse --show-toplevel)/contracts/simulation" && SIMULATION_WEEKS=512 forge test --match-test test_swarm -vvv --gas-limit 999999999999
```
...
```bash
cd "$(git rev-parse --show-toplevel)/contracts/simulation" && python3 scripts/generate_vbtc_ratio_chart.py
```
...
echo "| ${RUN_ID} | $(date +%Y-%m-%d) | test_swarm | 512 | 42 | 100 | success |" >> .claude/skills/simulation-results/index.md
```
