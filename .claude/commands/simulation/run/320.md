Run the simulation skill to execute a **320-week** (default) swarm simulation.
...
```bash
cd "$(git rev-parse --show-toplevel)/contracts/simulation" && forge test --match-test test_swarm -vvv --gas-limit 999999999999
```
...
echo "| ${RUN_ID} | $(date +%Y-%m-%d) | test_swarm | 320 | 42 | 100 | success |" >> .claude/skills/simulation-results/index.md
```
