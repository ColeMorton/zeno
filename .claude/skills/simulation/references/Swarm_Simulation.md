...
| `SimCurvePool` | Constant-product AMM for vBTC/WBTC with ratio ceiling at 1.0 | Mock contract |
| `MockCurvePool` | Controllable `price_oracle()` for PerpetualVault | Mock contract |
| `MockTWAPOracle` | Controllable `getTWAP()` for VarianceOracle | Mock contract |

---

## 3. Price Model

The simulation uses a **Geometric Brownian Motion (GBM)** model with **regime switching** for the WBTC/USDC price, and a **mean-reverting Ornstein-Uhlenbeck process** for the vBTC/WBTC ratio.
...
### vBTC/WBTC Ratio (Ornstein-Uhlenbeck)
...
The vBTC ratio feeds into the `MockCurvePool.price_oracle()` and `MockTWAPOracle.getTWAP()`, which are read by PerpetualVault and VarianceOracle respectively.

### SimCurvePool Ratio Ceiling

The `SimCurvePool` (agent-driven AMM) enforces a hard ceiling of 1.0 on vbtcRatio (`reserve0/reserve1`). Swaps that would push the ratio above 1.0 have their output capped to maintain parity. This models external arbitrage pressure — in reality, any exceedance above 1.0 creates risk-free profit via the separate-and-sell loop, which rational actors would immediately exploit. See `vBTC_Ratio_Upper_Bound.md` for the formal argument.
...
│   │   └── PriceSimulator.sol               # GBM + regime switching + O-U vBTC
│   └── mocks/
│       ├── MockCurvePool.sol                 # Controllable price_oracle()
│       └── MockTWAPOracle.sol                # Controllable getTWAP()
└── test/
